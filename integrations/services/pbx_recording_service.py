"""PBX call recordings: connector polls pending jobs, fetches via FTP, uploads to CRM storage."""
...
_AUDIO_SUFFIXES = (".wav", ".WAV", ".gsm", ".GSM", ".mp3", ".MP3")
...
def build_recording_ftp_path(recording_path: str) -> str:
    """
    Map ZYCOO CDR filesystem path to the path exposed by the PBX internal FTP server.

    CDR example:
      /var/spool/asterisk/monitor/recording/20260807/860/<file>.wav
    FTP path (verified on Zycoo):
      /recording/20260807/860/<file>.wav
    """
    path = (recording_path or "").strip().replace("\\", "/")
    if not path:
        return ""

    if path.startswith(("ftp://", "ftps://")):
        without_scheme = path.split("://", 1)[-1]
        slash = without_scheme.find("/")
        path = without_scheme[slash:] if slash >= 0 else ""
        if not path:
            return ""
    ...
    for prefix in (
        ...
        if path.startswith(prefix):
            ...
            if rest.startswith("recording/"):
                return f"/{rest}"
            return f"/recording/{rest}" if rest else ""

    if path.startswith("/"):
        # Unknown absolute path — only accept if it already looks like FTP layout.
        return path if "/recording/" in path else ""

    return f"/recording/{path.lstrip('/')}"
...
def apply_recording_path_from_cdr(
    ...
    *,
    ...
    """
    Store PBX filesystem path and FTP-relative fetch path; LAN connector polls jobs.
    Returns True if the recording was newly queued.
    """
    path = (recording_path or "").strip()
    if not path or not path.endswith(_AUDIO_SUFFIXES):
        return False

    if record.recording_uploaded and record.recording_status == PbxRecordingStatus.READY:
        return False

    ftp_path = build_recording_ftp_path(path)
    if not ftp_path:
        logger.warning(
            "PBX recording skipped (no FTP path) record_id=%s path=%s",
            record.id,
            path[:80],
        )
        return False
    ...
    if (
        ...
        and record.recording_url == ftp_path
    ...
    record.recording_path = path
    # Reuse recording_url to hold the FTP path until upload completes (then cleared).
    record.recording_url = ftp_path
    ...
    logger.info(
        "PBX recording queued for connector FTP poll record_id=%s ftp_path=%s",
        record.id,
        ftp_path[:120],
    )
    return True
...
def list_pending_recording_jobs(company_id: int) -> list[dict[str, Any]]:
    ...
    jobs = []
    for rec in qs:
        ftp_path = (rec.recording_url or "").strip()
        if not ftp_path.startswith("/recording/"):
            ftp_path = build_recording_ftp_path(rec.recording_path)
        if not ftp_path:
            continue
...
def finalize_recording_upload(
    *,
    ...
    # Clear FTP path from recording_url; playback uses signed CRM URL / storage key.
...
def get_playback_url(record: PbxCallRecord, request=None) -> str | None:
    ...
    if record.recording_status != PbxRecordingStatus.READY:
        return None
    if record.recording_url and record.recording_url.startswith(("http://", "https://")):
