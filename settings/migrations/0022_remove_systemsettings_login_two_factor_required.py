...
def migrate_platform_login_2fa_to_owners(apps, schema_editor):
    ...
    settings = SystemSettings.objects.using(db_alias).filter(pk=1).first()
    if not settings or getattr(settings, "login_two_factor_required", True):
        return

    owner_ids = list(
        Company.objects.using(db_alias).exclude(owner_id=None).values_list("owner_id", flat=True)
    )
    if owner_ids:
        User.objects.using(db_alias).filter(id__in=owner_ids).update(login_two_factor_enabled=False)
